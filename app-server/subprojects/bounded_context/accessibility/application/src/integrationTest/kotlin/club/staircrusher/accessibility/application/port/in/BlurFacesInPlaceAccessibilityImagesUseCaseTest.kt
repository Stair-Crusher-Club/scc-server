...
class BlurFacesInPlaceAccessibilityImagesUseCaseTest {
    ...
    @Autowired
    private lateinit var blurFacesInPlaceAccessibilityImagesUseCase: BlurFacesInPlaceAccessibilityImagesUseCase
    ...
    fun `PlaceAccessibility 이미지 중 얼굴이 감지된 사진만 업데이트한다`() {
        ...
        blurFacesInPlaceAccessibilityImagesUseCase.handle(placeAccessibilityId = placeAccessibility.id)
    ...
    @Test
    fun `이미 썸네일 처리가 된 PlaceAccessibility 의 경우 블러링 한 이미지 사용을 위해 썸네일 url을 제거한다`() {
        val (_, placeAccessibility, _) = registerPlaceAccessibilityAndBuildingAccessibility(
            listOf(MockDetectFacesService.URL_WITH_FACES, MockDetectFacesService.URL_WITHOUT_FACES)
        )
